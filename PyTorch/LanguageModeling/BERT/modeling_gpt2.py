...
class GPT2BiAttention(GPT2Attention):
...
class BiGPT2Block(nn.Module):
    def __init__(self, config):
        super(BiGPT2Block, self).__init__()
        hidden_size = config.hidden_size
        inner_dim = config.n_inner if config.n_inner is not None else 4 * hidden_size

        self.ln_1 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = GPT2BiAttention(config)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
...
class BiGPT2(GPT2PreTrainedModel):
    _keys_to_ignore_on_load_missing = ["attn.masked_bias"]

    def __init__(self, config):
        super(BiGPT2, self).__init__(config)
        ...
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([BiGPT2Block(config) for _ in range(config.num_hidden_layers)])
        self.ln_f = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
...
class GPT2Pooler(nn.Module):
    def __init__(self, config):
        ...
        self.dense_act = LinearActivation(config.n_embd, config.n_embd, act="tanh")
...
class GPT2ForPreTraining(GPT2PreTrainedModel):
    ...
    def forward(self,
                    ...
                    **kwargs):
        """
        Note we follow the HF model convention and did not annotate the specific argument type here.
        We want to ensure that we perform the NS loss only if the NS head is present. We accomplish this by not
        passing in the labels and next_sentence labels into HF so the loss is done in this forward only.
        """
        outputs = self.transformer.forward(input_ids=input_ids,
                                           attention_mask=attention_mask,
                                           token_type_ids=token_type_ids,
                                           position_ids=position_ids,
                                           head_mask=head_mask,
