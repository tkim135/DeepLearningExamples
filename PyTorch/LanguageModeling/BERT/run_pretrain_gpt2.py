...
from run_pretraining import BertPretrainingCriterion
...
def prepare_model_and_optimizer(args, device):
    ...
    # Prepare model
    config = modeling_gpt2.GPT2Config.from_json_file(args.config_file)
    ...
    if not args.resume_from_checkpoint:
        ...
        # global_step = args.resume_step if args.resume_step >= 0 else 0
...
def main():
    ...
    tensorboard = SummaryWriter(log_dir=os.path.join(args.output_dir, 'tb_log', 'train'))
    tensorboard.add_text(tag='config/training', text_string=str(args), global_step=0)
        ...
        while True:
            ...
            if not args.resume_from_checkpoint or epoch > 0 or (args.phase2 and global_step < 1) or args.init_checkpoint:
                files = [os.path.join(args.input_dir, f) for f in os.listdir(args.input_dir) if
                         os.path.isfile(os.path.join(args.input_dir, f)) and 'train' in f]
                files.sort()
                num_files = len(files)
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    prediction_scores = model(input_ids=input_ids, token_type_ids=None, attention_mask=input_mask)
                    ...
                    if global_step >= args.steps_this_run or timeout_sent:
                        ...
                        if is_main_process():
                            tensorboard.add_scalar("loss/avg_total", average_loss / (args.log_freq * divisor),
                                                   global_step=global_step)
